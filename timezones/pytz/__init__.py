...
if sys.version_info < (2, 3):
    from google3.pyglib.sets import Set as set
elif sys.version_info < (2, 4):
    # The exec wrapper prevents autopar from complaining in Python 2.2.
    exec 'from sets import Set as set'
...
def open_resource(name):
    ...
    for part in name_parts:
        if part == os.pardir or os.sep in part:
            raise ValueError('Bad path segment: %r' % part)
    filename = os.path.join('zoneinfo', *name_parts)
    if resources is not None:
        return resources.GetResourceAsFile(
            os.path.join('google3', 'third_party', 'py', 'pytz', filename))
    else:
